...
class pSp(nn.Module):
    ...
    def set_encoder(self):
        if self.opts.encoder_type == 'GradualStyleEncoder':
            # encoder = psp_encoders.GradualStyleEncoder(self.opts.encoder_number_layers, 'ir_se', self.opts)
            encoder = psp_encoders.GradualStyleEncoder(50, 'ir_se', self.opts)
        elif self.opts.encoder_type == 'Encoder4Editing':
            # encoder = psp_encoders.Encoder4Editing(self.opts.encoder_number_layers, 'ir_se', self.opts)
            encoder = psp_encoders.Encoder4Editing(50, 'ir_se', self.opts)
        elif self.opts.encoder_type == 'SingleStyleCodeEncoder':
            # encoder = psp_encoders.BackboneEncoderUsingLastLayerIntoW(self.opts.encoder_number_layers, 'ir_se', self.opts)
            encoder = psp_encoders.BackboneEncoderUsingLastLayerIntoW(50, 'ir_se', self.opts)
        elif self.opts.encoder_type == 'WPlusEncoder':
            # encoder = psp_encoders.BackboneEncoderUsingLastLayerIntoWPlus(self.opts.encoder_number_layers, 'ir_se', self.opts)
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            if self.opts.train_decoder:
                print('train decoder!!!!')
                self.decoder = torch.load(os.path.abspath(self.opts.stylegan_weights)).requires_grad_(True).to(self.opts.device)
            else:
                self.decoder = torch.load(os.path.abspath(self.opts.stylegan_weights)).to(self.opts.device)
    ...
    def __load_latent_avg(self, repeat=None):
        ...
        # w_samples = self.decoder.mapping(torch.from_numpy(z_samples).cuda(), None)  # [N, L, C]
