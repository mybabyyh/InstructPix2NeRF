...
def vis_faces_with_id(hooks_dict, fig, gs, i):
	...
	if 'prompt' in hooks_dict :
		prompt = hooks_dict["prompt"]
		# prompt = ''
		# prompt_init = hooks_dict["prompt"].strip()
		# prompt_rate = len(prompt_init) // 32
		# prompt_rate = prompt_rate + 1 if len(prompt_init) % 32 != 0 else prompt_rate
		# for i in range(prompt_rate):
		# 	prompt += prompt_init[i*32: (i+1)*32] + "\\n"
		# print(prompt)
